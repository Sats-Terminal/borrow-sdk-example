...
export function LoanDetailsDialog({
  ...
  const [collateralToWithdrawAfterRepay, setCollateralToWithdrawAfterRepay] =
    useState("");
  ...
  const [repaySourceAddress, setRepaySourceAddress] = useState("");
  const [repaySourcePositions, setRepaySourcePositions] = useState<any>(null);
  const [repaySourceLoading, setRepaySourceLoading] = useState(false);

  const loadRepaySourceData = useCallback(
    async (loanChain: string) => {
      ...
      try {
        ...
        console.error(
          "[LoanDetailsDialog] Failed to load repay source data:",
          err,
        );
        setRepaySourceAddress("");
        setRepaySourcePositions(null);
      } finally {
        setRepaySourceLoading(false);
      }
    },
    [getBaseAddressForChain, getWalletPositionsForChain],
  );
      ...
      getLoanCollateralInfo(loan.id)
        ...
        .catch((err) => {
          ...
          toast({
            ...
            description:
              "Could not retrieve collateral details. Please try again.",
            variant: "destructive",
          });
        })
        .finally(() => setLoadingInfo(false));

      void loadRepaySourceData(loan.borrowTransaction?.chain || "BASE");
  ...
  }, [open, loan, btcAddress, getLoanCollateralInfo, loadRepaySourceData]);
  ...
  const totalCollateralBtc = collateralInfo
    ? parseCollateralValue(collateralInfo.totalCollateral)
    : 0;
  const maxWithdrawableBtc = collateralInfo
    ? parseCollateralValue(collateralInfo.maxWithdrawable)
    : 0;
  ...
  const maxRepayAmount = walletPositionsLoaded
    ...
    : remainingDebt;
  const hasEnoughRepayBalance =
    repayAssetBalance >= remainingDebt && remainingDebt > 0;
  ...
  const healthFactor =
    remainingDebt > 0 ? (collateralValue * 0.8) / remainingDebt : 0;
  const healthColor =
    healthFactor > 1.5
      ? "text-green-500"
      : healthFactor > 1.2
  ...
  const handleRepay = async () => {
    try {
      ...
      const hasCollateralWithdraw =
        withdrawAfterRepay && parseFloat(collateralToWithdrawAfterRepay) > 0;
      ...
      console.log("[handleRepay] Repaying from source wallet:", {
        loanId: loan.id,
        chain,
        repayAmount,
        remainingDebt,
      ...
      toast({
        ...
        description:
          err instanceof Error ? err.message : "Failed to process repayment",
        variant: "destructive",
      });
    }
  };
  ...
  const handleWithdrawCollateral = async () => {
    ...
    try {
      const txId = await withdrawCollateral(
        loan.id,
        withdrawBtcAmount,
        withdrawBtcAddress,
        chain,
      ...
      toast({
        ...
        description:
          err instanceof Error ? err.message : "Failed to withdraw collateral",
        variant: "destructive",
      });
    }
  };
  ...
  const handleWithdrawUsdc = async () => {
    ...
    if (
      !withdrawUsdcAddress.startsWith("0x") ||
      withdrawUsdcAddress.length !== 42
    ) {
      toast({
    ...
    try {
      const txId = await withdrawToEVM(
        chain as any,
        withdrawUsdcAmount,
        withdrawUsdcAddress,
      );
      ...
      toast({
        ...
        description:
          err instanceof Error ? err.message : "Failed to transfer USDC",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        {/* Repay wallet address for the current loan chain */}
        {repayWalletAddress && (
          <div className="flex items-center gap-2 p-2.5 bg-zinc-50 rounded-lg border-[0.5px] text-sm">
            <span className="text-muted-foreground">Repay Wallet:</span>
            <code className="font-mono text-xs">
              {repayWalletAddress.slice(0, 10)}...{repayWalletAddress.slice(-8)}
            </code>
            <Button
              variant="ghost"
              size="sm"
              className="h-6 w-6 p-0"
              onClick={() => copyAddress(repayWalletAddress)}
            >
              {copiedAddress ? (
        ...
        <Tabs defaultValue="repay" className="mt-2">
          ...
          <TabsContent value="repay" className="mt-4 space-y-4">
              ...
              <div className="p-3 bg-zinc-50 rounded-xl border-[0.5px] space-y-2">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Index-0 smart account on {repaymentChainLabel}
                    </p>
                  </div>
                  ...
                  <Button
                    ...
                    onClick={() => void loadRepaySourceData(chain)}
                    disabled={repaySourceLoading}
                  >
                    {repaySourceLoading ? (
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Repaying from smart account on {repaymentChainLabel}:{" "}
                {repayWalletAddress
                  ? `${repayWalletAddress.slice(0, 10)}...${repayWalletAddress.slice(-8)}`
                  : "Not set"}
              </p>
              {showRepayBalanceWarning && (
                <p className="text-xs text-amber-600">
                  Entered amount is above your current repay-source{" "}
                  {repaymentAsset} balance on {repaymentChainLabel}.
                </p>
              )}
            </div>
            ...
            {isFullRepayment && (
              <div className="space-y-3 p-3 border-[0.5px] rounded-xl">
                ...
                {withdrawAfterRepay && (
                  <div className="space-y-2">
                    <Label htmlFor="collateralAfterRepay">
                      Collateral to Withdraw (BTC)
                    </Label>
                    <div className="relative">
                      <Input
                        ...
                        onChange={(e) =>
                          setCollateralToWithdrawAfterRepay(e.target.value)
                        }
                        className="h-10"
                        step="0.0001"
                        min="0"
            ...
            <Button
              onClick={handleRepay}
              disabled={
                repaying || !repayAmount || parseFloat(repayAmount) <= 0
              }
              variant="accent"
          ...
          <TabsContent value="withdraw-btc" className="mt-4 space-y-4">
            <div className="p-3 bg-zinc-50 rounded-xl border-[0.5px]">
              ...
              <p className="text-xl font-semibold font-mono">
                {maxWithdrawableBtc.toFixed(8)} BTC
              </p>
              <p className="text-xs text-muted-foreground font-mono">
                ...
                <div className="text-sm text-blue-700">
                  ...
                  <p>
                    ...
                    {remainingDebt < 0.01 ? (
                      <span>
                        {" "}
                        This small amount prevents full collateral withdrawal.{" "}
                        <strong>Repay all debt first</strong> to withdraw your
                        full collateral.
                      </span>
                    ) : (
                      <span>
                        {" "}
                        Repay your debt to increase the amount you can withdraw.
                      </span>
                    )}
                  </p>
                </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Button
                  ...
                  className="absolute right-1 top-1/2 -translate-y-1/2"
                  onClick={() =>
                    setWithdrawBtcAmount(maxWithdrawableBtc.toFixed(8))
                  }
                >
                  Max
                </Button>
            ...
            <div className="flex items-start gap-2.5 p-3 bg-amber-500/5 border-[0.5px] border-amber-200 rounded-xl">
              ...
              <p className="text-sm text-amber-700">
                Withdrawing collateral will reduce your health factor and may
            ...
            <Button
              ...
              disabled={
                ...
                !withdrawBtcAmount ||
                !withdrawBtcAddress ||
                parseFloat(withdrawBtcAmount) <= 0 ||
                parseFloat(withdrawBtcAmount) > maxWithdrawableBtc
              }
              variant="accent"
              className="w-full h-10"
            >
          ...
          <TabsContent value="withdraw-usdc" className="mt-4 space-y-4">
            <div className="p-3 bg-zinc-50 rounded-xl border-[0.5px] flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">
                  Transfer USDC from Repay Wallet
                </p>
                <p className="text-xs text-muted-foreground">
                  Up to ${loanAmount.toLocaleString()} USDC (original loan)
                </p>
              </div>
              ...
              <div className="p-2.5 bg-blue-500/5 border-[0.5px] border-blue-200 rounded-lg">
                <p className="text-xs text-blue-700">
                  <span className="font-medium">
                    From {repaymentChainLabel} Repay Wallet:
                  </span>
                  <code className="ml-1 font-mono">
                    {repayWalletAddress.slice(0, 10)}...
                    {repayWalletAddress.slice(-8)}
                  </code>
                </p>
              </div>
            )}
            ...
            <div className="space-y-2">
              <Label htmlFor="withdrawUsdcAddress">
            ...
            <div className="p-3 bg-green-500/5 border-[0.5px] border-green-200 rounded-xl">
              ...
              <p className="text-sm text-green-600 mt-1">
                Transfer USDC from your repay wallet without paying gas fees.
              </p>
            </div>
            ...
            <Button
              ...
              disabled={
                ...
                !withdrawUsdcAmount ||
                !withdrawUsdcAddress ||
                parseFloat(withdrawUsdcAmount) <= 0
              }
              variant="accent"
              className="w-full h-10"
            >
